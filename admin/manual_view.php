    ...
    <section class="content">
      ...
      <div class="card">
              <div class="card-header">
                <h3 class="card-title">Data manually added.</h3>
              </div>
              <!-- /.card-header -->
...
<?php
$conn = $pdo->open();
$stmt = $conn->prepare("SELECT * FROM news WHERE input=:input AND NOT post_status=1 ORDER BY id DESC");
...
foreach($scrape as $row){
   echo '
                    ...
                    <td>'.$row['category'].'</td>
                    <td>'.$row['time'].'</td>
                    <td><img src="../images/'.$row['photo'].'" width="150px" /></td>
