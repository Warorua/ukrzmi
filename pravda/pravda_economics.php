...
if (!filter_var($img_f, FILTER_VALIDATE_URL) === false) {
    $image = $img_f;
   // $output .=("$pic_1 is a valid URL");
} elseif(!filter_var($datt['image']['url'], FILTER_VALIDATE_URL) === false) {
    $image = $datt['image']['url'];
...
//insert into database
  ...
  $stmt->execute(['sub_1'=>$sub_1, 'sub_2'=>$sub_2, 'source_error'=>$ar_error, 'video_url'=>$video_f, 'type'=>$news_type, 'parent'=>$parent, 'source'=>"epravda.com.ua/", 'deep_link'=>$h_link, 'title'=>$title, 'published'=>$published, 'author'=>$p_author, 'article'=>$ar_full, 'tag_1'=>$tag1, 'tag_2'=>$tag2, 'tag_3'=>$tag3, 'photo'=>$filename, 'photo_url'=>$image, 'p_grapher'=>$photographer, 'category'=>mb_convert_encoding("Економіка", "utf-8"), 'time'=>$time, 'code'=>$code]);
