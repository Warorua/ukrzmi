...
////////////////////////////////////////////////////////////////////////////////////////
///////////////         HOME PAGE TRENDING            ///////////////////////////////
$link = 'https://www.unian.ua/detail/opinions';
$output = '';
//Title
...
//Link HREF
$href =  $response->find('div.author-round div.author-round__item div.author-round__info h3 a[href]');
$f_href = $href[0]->href;
$output .= $f_href.'<br/>';
...
//Date
$date =  $content->find('div.infinity-item div.article div.article__info div div.article__info-item');
...
//Author Fetch
$author = $content->find('div.infinity-item div.article span.article__author a');
...
//profile pic
$filee2 = basename($pic_1);
$img2 = 'prof_'.$gen.".jpg";
$path2 = '../images/'.$img2; 
file_put_contents($path2, file_get_contents($pic_1));
$voice_profile = $img2;


$parent = "unian.ua"; 
//insert into database
  $stmt = $conn->prepare("INSERT INTO news (voice_profile, sub_1, sub_2, source_error, video_url, type, parent, source, deep_link, title, published, author, article, tag_1, tag_2, tag_3, photo, photo_url, p_grapher, category, time, code) VALUES (:voice_profile, :sub_1, :sub_2, :source_error, :video_url, :type, :parent, :source, :deep_link, :title, :published, :author, :article, :tag_1, :tag_2, :tag_3, :photo, :photo_url, :p_grapher, :category, :time, :code)");
  $stmt->execute(['voice_profile'=>$voice_profile, 'sub_1'=>$sub_1, 'sub_2'=>$sub_2, 'source_error'=>$ar_error, 'video_url'=>$video_f, 'type'=>$news_type, 'parent'=>$parent, 'source'=>"Unian.ua/detail/opinions", 'deep_link'=>$f_href, 'title'=>$title, 'published'=>$published, 'author'=>$p_author, 'article'=>$ar_full, 'tag_1'=>$tag1, 'tag_2'=>$tag2, 'tag_3'=>$tag3, 'photo'=>$filename, 'photo_url'=>$image, 'p_grapher'=>"None", 'category'=>$p_cat, 'time'=>$time, 'code'=>$code]);
