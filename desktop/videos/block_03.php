...
<div style="background-color:<?php echo $block[2]['bg_color']; ?>" class="row cardPanel pb-2">
 ...
 <button id="carPrev<?php echo $block[2]['id']; ?>" type="button" data-role="" class="slickk-prev-<?php echo $block[2]['name']; ?> btn btn-outline-dark btn-sm" <?php if(isset($hide_control_button)){echo $hide_control_button;} ?>>Previous</button>
  
<button id="carNext<?php echo $block[2]['id']; ?>" type="button" data-role="" class="slickk-next-<?php echo $block[2]['name']; ?> btn btn-outline-dark btn-sm" <?php if(isset($hide_control_button)){echo $hide_control_button;} ?>>Next</button>
  </div>
...
$('.carousel-<?php echo $block[2]['name']; ?>').on('init',function(event, slick){
  document.getElementById('carPrev<?php echo $block[2]['id']; ?>').style.visibility = 'hidden';
});
...
$('.carousel-<?php echo $block[2]['name']; ?>').on('afterChange', function(event, slick, currentSlide){
  ...
  if (CarNo==0) {
    ...
    document.getElementById('carPrev<?php echo $block[2]['id']; ?>').style.visibility = '';
  }
  if (CarNo==<?php echo $slide_control; ?>) {
    //alert('this is page 0')
    document.getElementById('carNext<?php echo $block[2]['id']; ?>').style.visibility = 'hidden';
  }
else if(CarNo!=<?php echo $slide_control; ?>){
    document.getElementById('carNext<?php echo $block[2]['id']; ?>').style.visibility = '';
  }
