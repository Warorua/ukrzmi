...
<h2 class="newsHead"><?php echo $block[1]['name']; ?></h2>
<div style="background-color:<?php echo $block[1]['bg_color']; ?>" class="cardBlock row carousel-<?php echo $block[1]['name']; ?>">
...
<div style="background-color:<?php echo $block[1]['bg_color']; ?>" class="row cardPanel pb-2">
 ...
 <button id="carPrev<?php echo $block[1]['id']; ?>" type="button" data-role="" class="slickk-prev-<?php echo $block[1]['name']; ?> btn btn-outline-dark btn-sm" <?php if(isset($hide_control_button)){echo $hide_control_button;} ?>>Previous</button>
...
$('.carousel-<?php echo $block[1]['name']; ?>').on('init',function(event, slick){
  document.getElementById('carPrev<?php echo $block[1]['id']; ?>').style.visibility = 'hidden';
});
...
$('.carousel-<?php echo $block[1]['name']; ?>').on('afterChange', function(event, slick, currentSlide){
  ...
  if (CarNo==0) {
    ...
    document.getElementById('carPrev<?php echo $block[1]['id']; ?>').style.visibility = '';
  }
  if (CarNo==<?php echo $slide_control; ?>) {
    //alert('this is page 0')
    document.getElementById('carNext<?php echo $block[1]['id']; ?>').style.visibility = 'hidden';
  }
else if(CarNo!=<?php echo $slide_control; ?>){
    document.getElementById('carNext<?php echo $block[1]['id']; ?>').style.visibility = '';
  }
