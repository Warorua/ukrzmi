...
<div style="background-color:<?php echo $block[$block_id]['bg_color']; ?>" class="cardBlock carousel-<?php echo $block[$block_id]['name']; ?>">
...
<div style="background-color:<?php echo $block[$block_id]['bg_color']; ?>" class="cardBlock container">

  <div class="d-flex justify-content-between">
   <button id="carPrev<?php echo $block[$block_id]['name']; ?>" type="button" data-role="" class="slickk-prev-<?php echo $block[$block_id]['name']; ?> btn btn-outline-dark btn-sm w-25" <?php if(isset($hide_control_button)){echo $hide_control_button;} ?>>Previous</button>
  <button id="carNext<?php echo $block[$block_id]['name']; ?>" type="button" data-role="" class="slickk-next-<?php echo $block[$block_id]['name']; ?> btn btn-outline-dark btn-sm w-25" <?php if(isset($hide_control_button)){echo $hide_control_button;} ?>>Next</button>
  </div>
...
$('.carousel-<?php echo $block[$block_id]['name']; ?>').on('init',function(event, slick){
  document.getElementById('carPrev<?php echo $block[$block_id]['name']; ?>').style.display = 'none';
});
...
$('.carousel-<?php echo $block[$block_id]['name']; ?>').on('afterChange', function(event, slick, currentSlide){
  ...
  if (CarNo==0) {
    ...
    document.getElementById('carPrev<?php echo $block[$block_id]['name']; ?>').style.display = '';
  }
  if (CarNo==<?php echo $slide_control; ?>) {
    //alert('this is page 0')
    document.getElementById('carNext<?php echo $block[$block_id]['name']; ?>').style.display = 'none';
  }
  else if(CarNo!=<?php echo $slide_control; ?>){
    document.getElementById('carNext<?php echo $block[$block_id]['name']; ?>').style.display = '';
  }
