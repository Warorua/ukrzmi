...
if($author_size > 1){
    foreach($author as $autho){
        $p_author = $autho->plaintext;
    }
}
else{
    $p_author = "кореспондент";
}
...
//insert into database
  ...
  $stmt->execute(['sub_1'=>$sub_1, 'sub_2'=>$sub_2, 'source_error'=>$ar_error,  'parent'=>$parent, 'source'=>"ua.korrespondent.net/sport/", 'deep_link'=>$f_href, 'title'=>$title, 'published'=>$published, 'author'=>$p_author, 'article'=>$ar_full, 'tag_1'=>$tag1, 'tag_2'=>$tag2, 'tag_3'=>$tag3, 'photo'=>$filename, 'photo_url'=>$image, 'p_grapher'=>$photographer, 'category'=>"Спорт", 'time'=>$time, 'code'=>$code]);
